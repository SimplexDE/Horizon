...
public class PlayerConfig {
    private static final String path = "plugins/Horizon/players/%s.yml", tP = "token";
    YamlConfiguration con;
    File file;

    public PlayerConfig(final OfflinePlayer p) {
        final String uuid = p.getUniqueId().toString();
        this.file = new File(String.format(path, uuid));
        this.con = YamlConfiguration.loadConfiguration(this.file);
    }

    public static PlayerConfig loadConfig(final OfflinePlayer p) {
        if (!hasConfig(p)) {
            createConfig(p);
        }
        return new PlayerConfig(p);
    }

    public static PlayerConfig createConfig(final OfflinePlayer p) {
        final String uuid = p.getUniqueId().toString();
        final File file = new File(String.format(path, uuid));
        if (!file.exists()) {
            try {
                Utils.createFolder("plugins/Horizon/players");
                file.createNewFile();
            } catch (IOException e) {
                e.printStackTrace();
            }
